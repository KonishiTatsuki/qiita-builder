...
describe("getClubsName", () => {
    ...
    it('returns the correct club name for a given id', async () => {
        // 実行
        const clubName = await wrapper.vm.getClubsName(133);
    
        // 検証
        expect(clubName).toEqual('バレー');
    });

    it('returns empty string if id is not found', async () => {
        // 実行
        const clubName = await wrapper.vm.getClubsName(0);
        
        // 検証
        expect(clubName).toEqual('');
    });
});